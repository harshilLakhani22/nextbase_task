...
const worker = new Worker(
  ...
  async (job: any) => {
    ...
    try {
      ...
      await ThumbnailJob.updateOne(
        { jobId },
        { status: 'completed', thumbnailPath: savedThumbnailPath, thumbnailUrl: thumbnailRelativeUrl }
      );

      // publish a compact payload with a relative thumbnailUrl (frontend will prefix with origin)
      await loggerRedis.publish(
        pubChannel,
        JSON.stringify({ jobId, userId, status: 'completed', thumbnailPath: savedThumbnailPath, thumbnailUrl: thumbnailRelativeUrl })
      );
...
worker.on('active', async (job: any) => {
  try {
